...
def plot_clusters(
    X, labels, medoids=None, feature_names=None, title="Clustering Results"
):
...
def main():
    ...
    try:
        ...
        plot_clusters(
            ...
            title="K-Medoids Clustering (Iris)",
        )
    ...
    plot_clusters(
        ...
        agglo_labels,
        feature_names=feature_names,
        title="Agglomerative Clustering (Iris)",
    )
    plot_dendrogram(
        X,
        title="Agglomerative Dendrogram (last 30 merges)",
        truncate_mode="lastp",
        p=30,
    )
    ...
    # Compare clusterings
    print(
        "Adjusted Rand Index between K-Medoids and Agglomerative:",
        (
            adjusted_rand_score(kmedoids_labels, agglo_labels)
            if "kmedoids_labels" in locals()
            else "N/A"
        ),
    )
